...
def crawl_web(seed):
    tocrawl = [seed]
    crawled = []
    index = []    # Modifying the crawl_web procedure by introducing the "index" list as use it as we go and crawl the web starting from seed page
    while tocrawl:
        page = tocrawl.pop()
        if page not in crawled:
            content = get_page(page)    # Storing the "expensinve to run" get_page(page) procedure in a variable
            add_page_to_index(index,page,content)    # Run the add_page_to_index(index,page,content) procedure with every new page found
            union(tocrawl,get_all_links(content))
            crawled.append(page)
    return index    # We want to return "index" to actually get the "index" list of content keywords matched with their associated links
